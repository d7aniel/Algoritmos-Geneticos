class Cromosoma {
  ...
  float getValorMapeado(int[]serie, float cual) {
    ...
    //float valorMap = int(map(valorRaw, 0, 255, 0, cant+1));
    //valorMap = valorRaw;//cant>=255?valorRaw:map(valorMap, 0, cant+1, 0, 255);
    return valor;
  }
  ...
  int[] sinCeros(int[] arreglo) {
    int largo = 0;
    for (int i=0; i<arreglo.length; i++) {
      if (arreglo[i] != 0)
        largo++;
    }
    int [] arregloNuevo = new int[largo];
  ...
  /* float[] mapa256(float[] arreglo) {
   float[] nuevoArreglo = arreglo;//new float[arreglo.length];
   float cuenta = img.width*img.height;
   int hack = 0;
   while (cuenta!=255 && hack<10000) {
   float cuenta1 = 0;
   for (int i=0; i<arreglo.length; i++) {
   nuevoArreglo[i] = nuevoArreglo[i]==0?0:(nuevoArreglo[i]*255)/(cuenta);
   cuenta1+=nuevoArreglo[i];
   }
   println(cuenta1);
   cuenta = cuenta1;
   hack++;
